...
const _session = new Soup.Session(); _session.set_timeout(10);
function fetchJson(url, cb) {
    let msg = Soup.Message.new('GET', url);
    _session.send_and_read_async(msg, GLib.PRIORITY_DEFAULT, null, (s,r) => {
        try { let b=s.send_and_read_finish(r); let t=imports.byteArray.toString(b.get_data()); cb(JSON.parse(t),null); }
        catch(e){cb(null,e);}
    });
}
...
function getWeather(lat, lon, cb) {
    const url=`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,weather_code,wind_speed_10m,precipitation&daily=weather_code,temperature_2m_max,temperature_2m_min&forecast_days=5&timezone=auto`;
...
function createWeatherBox() {
    ...
    if (display) {
        ...
        css.load_from_data(`
            .weather-frame { padding: 8px; }
            .weather-current { border: 1.5px solid @secondary; border-radius: 12px; padding: 10px 14px; margin-bottom: 6px; }
            .weather-icon-lg { font-size: 2.8em; color: @primary; margin-end: 4px; }
            .weather-temp-lg { font-size: 2em; font-weight: 700; color: @primary; }
            .weather-desc { font-size: 1em; font-weight: 600; color: @primary; }
            .weather-loc { font-size: 0.85em; font-weight: 500; color: @primary; opacity: 0.7; }
            .weather-detail { font-size: 0.8em; color: @primary; opacity: 0.8; }
            .weather-forecast { border: 1px solid @secondary; border-radius: 10px; padding: 8px 4px; margin-bottom: 6px; }
            .fc-day { font-size: 0.75em; font-weight: 600; color: @primary; opacity: 0.7; }
            .fc-icon { font-size: 1.3em; color: @primary; }
            .fc-hi { font-size: 0.78em; font-weight: 600; color: @primary; }
            .fc-lo { font-size: 0.72em; color: @primary; opacity: 0.6; }
        `,-1);
        Gtk.StyleContext.add_provider_for_display(display,css,Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION);
    }

    const mainBox = new Gtk.Box({orientation:Gtk.Orientation.VERTICAL,spacing:6,margin_top:4,margin_bottom:4,margin_start:4,margin_end:4});
    mainBox.add_css_class('weather-frame');

    // ── Current weather ──
    const curBox = new Gtk.Box({orientation:Gtk.Orientation.HORIZONTAL,spacing:10,halign:Gtk.Align.CENTER,valign:Gtk.Align.CENTER});
    ...
    // ── Forecast grid ──
    const fcBox = new Gtk.Box({orientation:Gtk.Orientation.HORIZONTAL,spacing:4,halign:Gtk.Align.CENTER});
    fcBox.add_css_class('weather-forecast');
    const fcD=[],fcI=[],fcH=[],fcL=[];
    for (let i=0;i<5;i++) {
        const col=new Gtk.Box({orientation:Gtk.Orientation.VERTICAL,spacing:2,halign:Gtk.Align.CENTER,hexpand:true});
    ...
    function _start() {
        refresh();
        if(!_tid) _tid=GLib.timeout_add(GLib.PRIORITY_DEFAULT,60000,()=>{refresh();return GLib.SOURCE_CONTINUE;});
        if(!_utid) _utid=GLib.timeout_add(GLib.PRIORITY_DEFAULT,3000,()=>{let u=getUnit();if(u!==_lastUnit)refresh();return GLib.SOURCE_CONTINUE;});
    }
