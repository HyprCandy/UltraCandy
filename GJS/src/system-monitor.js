    ...
    try {
        for (let i=0;i<16;i++) {
            ...
            if (d.includes('amdgpu') || d.includes('radeon')) {
                ...
                // Try multiple usage metrics
                try {
                    let bp=`/sys/class/drm/card${i}/device/gpu_busy_percent`;
                    if (GLib.file_test(bp,GLib.FileTest.EXISTS)) {
                        let [ok,c]=GLib.file_get_contents(bp);
                        if (ok) usage = parseInt(imports.byteArray.toString(c).trim());
                    }
                } catch(e){}
                
                // Fallback to memory usage if GPU busy not available
                if (isNaN(usage) || usage === 0) {
                    try {
                        let [tok,total] = GLib.file_get_contents(`/sys/class/drm/card${i}/device/mem_info_vram_total`);
                        let [uok,used] = GLib.file_get_contents(`/sys/class/drm/card${i}/device/mem_info_vram_used`);
                        if (tok && uok) {
                ...
                try {
                    ...
                    for (let tempPath of tempPaths) {
                        let [tok,tc]=GLib.file_get_contents(tempPath);
                        if(tok){
                ...
                try {
                    let [nok,nc]=GLib.file_get_contents(`/sys/class/drm/card${i}/device/product_name`);
                    if(nok){
                ...
                // Fallback to lspci name
                try {
                    let [ok,addr] = GLib.file_get_contents(`/sys/class/drm/card${i}/device/address`);
                    if (ok) {
