...
/**
 * Setup Candy desktop entry and icons
 */
function setupCandyDesktop() {
    // Always regenerate to ensure launcher script is set
    ...
    try {
        ...
        // Also create Candy.svg for fallback
...
function toggleUtils() {
    if (!widgets.utils) {
        ...
        // Set window class to 'Candy' for dock icon matching
...
function onActivate() {
    ...
    app.hold();  // Keep application running
    
    // CSS hot reload via file monitoring only (no GSettings to avoid flicker)
